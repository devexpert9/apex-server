  ...
  users.findOne({email: req.body.email, '_id': { $ne: req.body._id}}, function(err, user)
  {
    if(user == null)
    {
      users.findOne({username: req.body.username, '_id': { $ne: req.body._id}}, function(err, username)
      {
        ...
        {
          console.log(req.body)
          users.update({_id: req.body._id},{ $set: { 'name': req.body.name, 'username': req.body.username, 'email':req.body.email, 'password': req.body.password, 'contact': req.body.contact, 'zip': req.body.zip, 'state': req.body.state, 'city': req.body.city, 'country': req.body.country, 'address': req.body.address, 'expiry_date': req.body.expiry_date, 'created_on': new Date() } }, {new: true}, function(err, user)
          {
            if(user == null)
            {
              res.send({
